...
export function SummaryStep() {
  ...
  const generateReport = () => {
    ...
    report += QUESTIONS.map(q => {
      if (q.inputType === 'order_manager') return null; // Skip order here, handle separately
    ...
    // Order section for plain text
    if (orderItems.length > 0) {
      report += '\nORDINE / PREVENTIVO\n';
      report += '--------------------------------------------------\n';
      report += 'CODICE | DESCRIZIONE | QTA | NETTO | TOTALE\n';
      report += '--------------------------------------------------\n';
      ...
      orderItems.forEach(item => {
        ...
        grandTotal += total;
        // Simple fixed width attempt for plain text, though varying lengths break it easily. 
        // Just comma separated or simple format is safer.
        report += `${item.code} | ${item.description} | x${item.quantity} | €${netPrice} | €${total}\n`;
      });
      report += '--------------------------------------------------\n';
      report += `TOTALE ORDINE: €${grandTotal.toFixed(2)}\n`;
    }
  ...
  const handleEmail = () => {
    ...
    // Try to extract company name for subject
    let clientName = 'Nuovo Meeting';
    try {
        const cData = JSON.parse(answers['company_referent'] || '{}');
        if (cData.company) clientName = cData.company;
    } catch {}

    const subject = `Meeting Report: ${clientName}`;
    const body = encodeURIComponent(text);
    
    // Mailto is text only. HTML body in mailto is not standard and rarely works.
    // We stick to the plain text version generated above.
